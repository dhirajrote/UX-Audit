...
- All 8 scoring models (Percentage, 5-Star, Numeric, Weighted, Pass/Fail, Maturity, UX Health, Overall Experience) are implemented via one shared scoring function — the audit run screen shows the template's primary model prominently plus a "show all" toggle with every model's value for transparency.
